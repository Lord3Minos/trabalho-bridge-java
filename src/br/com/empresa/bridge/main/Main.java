...
public class Main {
    public static void main(String[] args) {
        ...
        // 1. Demonstração das combinações obrigatórias
        System.out.println("\n--- 1. Executando Combinações Obrigatórias ---");
        
        Documento relatorio = new RelatorioGerencial(GeradorFactory.criarGerador("PDF"), "KPIs de Vendas", "Aumento de 20%");
        relatorio.gerar();

        Documento proposta = new PropostaComercial(GeradorFactory.criarGerador("HTML"), "Consultoria TI", 5000.00);
        proposta.gerar();

        Documento nf = new NotaFiscal(GeradorFactory.criarGerador("JSON"), "João Silva", Arrays.asList("Notebook", "Mouse"), 3550.00);
        nf.gerar();

        Documento contrato = new ContratoSimplificado(GeradorFactory.criarGerador("TXT"), "Empresa A & Empresa B", "Cláusula de Confidencialidade");
        contrato.gerar();

        // 2. Demonstração do Desafio Extra: Sistema Configurável
        ...
        switch (docOpcao) {
            case 1: docEscolhido = new RelatorioGerencial(geradorEscolhido, "Indicadores Custom", "Sucesso"); break;
            case 2: docEscolhido = new NotaFiscal(geradorEscolhido, "Cliente Custom", Arrays.asList("Item 1", "Item 2"), 100.0); break;
            case 3: docEscolhido = new PropostaComercial(geradorEscolhido, "Serviço Custom", 1000.0); break;
            case 4: docEscolhido = new ContratoSimplificado(geradorEscolhido, "Partes Custom", "Cláusulas Custom"); break;
        }
